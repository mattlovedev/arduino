// snek v4 — the decision algorithm, written out and then implemented.
//
// The comment block below is a SPEC: the move-picking logic inherited
// from snek/v3, in prose + pseudocode. Everything after the
// "IMPLEMENTATION" divider is that spec coded up in my own style — not
// a refactor of v1/v2. v4 starts as a byte-for-byte copy of v3; the
// AI changes land on top of it, tracked against the TO DO list below.
//
// =====================================================================
...
// =====================================================================
//
//   [x] Look past one cell: before committing to a move, confirm the
//       head still has a way out. Covered by the reachable-area check.
//   [x] Flood-fill / reachable-area check. chooseMove() only takes a
//       move if the open space it leads into is at least as large as
//       the snake. See reachableArea().
//   [x] Tail-chasing survival fallback: when no safe move leads toward
//       the food, chooseMove() aims for the tail instead. See
//       bestSafeToward() / chooseMove().
//   [x] Among near-equal safe moves, prefer the one that leaves the most
//       room open (don't cut the board in half), then the one that
//       keeps going straight. See bestSafeToward(). Wall-hugging is
//       skipped — it's a torus, and the room check covers the intent.
//   [x] Torus edges: keep them. The board wraps on both axes, and the
//       movement, distance, and flood-fill code all model it the same
//       way — the snake crosses an edge when that's the shorter route.
//       A deliberate choice, not a leftover. Solid walls would be a
//       different game; that's the v5 fork.
//
// =====================================================================
...
// IMPLEMENTATION
//
// My own coding of the spec above, then the v4 changes on top. Points
// to call out:
//
//  * v4: chooseMove() runs a flood-fill (reachableArea) and only takes
//    a move whose reachable open space still fits the whole snake. If no
//    such move heads toward the food it chases its own tail to stall
//    instead; only if nothing is safe does it fall back to v3 behaviour
//    (nearest open move toward the food, else die).
//  * v4: among safe moves that are within DIST_SLACK of the closest,
//    bestSafeToward() prefers the one opening into the most room (up to
//    ROOMY_ENOUGH), then the one that keeps going straight (lastDir).
...
//    it means v3/v4 and v2 can pick different moves when two directions
//    are equidistant.
...
struct Snake {
  ...
  int       head;
  int       len;
  Direction lastDir;   // direction of the last move; DirCount = none yet (v4)
...
// Move-chooser tuning (v4):
//   DIST_SLACK   - a safe move up to this many steps further from the
//                  target than the greedy best still counts as "as
//                  good", leaving a secondary preference to pick between
//                  them.
//   ROOMY_ENOUGH - reachable-area counting stops at len + this; past
//                  there a move is simply "wide open" and we no longer
//                  care which is roomier.
constexpr int DIST_SLACK   = 2;
constexpr int ROOMY_ENOUGH = 96;

// One step from `from` in `d`, wrapping each axis. The board is a torus
// by design (see TO DO); axisDistance() and reachableArea() model the
// same wrap. Only ever called after a single-cell move, so a branch per
// edge covers it.
...
// Flood-fill scratch, kept at file scope so a whole-board fill never
// lands on the AVR call stack.
static bool     seenCell[NUM_LEDS];
static uint16_t fillQueue[NUM_LEDS];

// Open cells reachable from `start`, walking the torus and treating the
// body (minus the tail, which vacates as we move) as walls. Counting
// stops at `limit`: the only question we ask is "is there room for the
// whole snake", so there's no point counting past that.
static int reachableArea(const Snake& s, Point start, int limit) {
  memset(seenCell, 0, sizeof(seenCell));
  int qHead = 0, qTail = 0, count = 0;

  auto enqueue = [&](Point p) {
    uint16_t idx = p.y * WIDTH + p.x;
    if (seenCell[idx] || s.hits(p)) return;
    seenCell[idx] = true;
    fillQueue[qTail++] = idx;
  };

  enqueue(start);
  while (qHead < qTail && count < limit) {
    uint16_t idx = fillQueue[qHead++];
    count++;
    Point p{ int(idx % WIDTH), int(idx / WIDTH) };
    for (int i = 0; i < DirCount; i++) enqueue(stepCell(p, Direction(i)));
  }
  return count;
}

// Best safe move toward `target`. "Safe" = the next cell is open and the
// space it leads into still fits the whole snake. Among the safe moves,
// the greedy pick is the one closest to the target; a move within
// DIST_SLACK of that is treated as equally close, and between those we
// prefer (c) the one that leaves the most room, then (a) the one that
// keeps going straight (lastDir). Returns DirCount if nothing is safe.
static Direction bestSafeToward(const Snake& s, Point target, Direction lastDir) {
  Point head = s.headCell();

  Direction ranked[DirCount];
  rankDirections(head, target, ranked);

  Direction cand[DirCount];
  int       n = 0;
  for (Direction d : ranked) {
    Point next = stepCell(head, d);
    if (!s.hits(next) && reachableArea(s, next, s.len) >= s.len)
      cand[n++] = d;
  }
  if (n == 0) return DirCount;
  if (n == 1) return cand[0];

  int bestDist = axisDistance(cand[0], head, target);
  int best     = 0;
  int bestArea = reachableArea(s, stepCell(head, cand[0]), s.len + ROOMY_ENOUGH);
  for (int i = 1; i < n; i++) {
    if (abs(axisDistance(cand[i], head, target) - bestDist) > DIST_SLACK) continue;
    int area = reachableArea(s, stepCell(head, cand[i]), s.len + ROOMY_ENOUGH);
    if (area > bestArea || (area == bestArea && cand[i] == lastDir)) {
      best     = i;
      bestArea = area;
    }
  }
...
// The move for this tick, in falling order of preference:
//   1. a safe route toward the food;
//   2. else stall: a safe route toward our own tail. The tail keeps
//      moving away, so this loops us around our body until a safe food
//      route reopens. Only reached by a long snake in a tight spot;
//   3. else the nearest open move toward the food;
//   4. else the top-ranked move, and we die on it (same as v3).
static Direction chooseMove(const Snake& s, Point food) {
  Direction d = bestSafeToward(s, food, s.lastDir);
  if (d != DirCount) return d;

  d = bestSafeToward(s, s.segment(s.len - 1), s.lastDir);
  if (d != DirCount) return d;

  Direction ranked[DirCount];
  rankDirections(s.headCell(), food, ranked);
  for (Direction e : ranked)
    if (!s.hits(stepCell(s.headCell(), e)))
      return e;
  return ranked[0];
}
